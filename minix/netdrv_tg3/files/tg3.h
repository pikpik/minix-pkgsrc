 ...
 *
 * Ported to Minix by Colin Fowler (elethiomel@gmail.com)
 *
 */
...
/* Minix. Big enough to hold an ethernet packet and should also pad well. */
#define TG3_TX_IOVEC_SIZE	2048
...
#define TG3_EMPTY		0x00
#define TG3_ENABLED		0x01
#define TG3_GOTREADV	0x02
#define TG3_GOTWRITEV	0x04

#define TG3_MULTI		0x08
#define TG3_BROAD		0x10
#define TG3_PROMISC		0x20
...
#define VLAN_HLEN       4	/* The additional bytes (on top of the
				 * Ethernet header) that VLAN requires. */
...
#define  SG_DIG_PARTNER_FAULT_MASK	 0x00600000	/* If
							 * !MRADV_CRC16_SELECT */
#define  SG_DIG_PARTNER_ASYM_PAUSE	 0x00100000	/* If
							 * !MRADV_CRC16_SELECT */
#define  SG_DIG_PARTNER_PAUSE_CAPABLE	 0x00080000	/* If
							 * !MRADV_CRC16_SELECT */
#define  SG_DIG_PARTNER_HALF_DUPLEX	 0x00040000	/* If
							 * !MRADV_CRC16_SELECT */
#define  SG_DIG_PARTNER_FULL_DUPLEX	 0x00020000	/* If
							 * !MRADV_CRC16_SELECT */
#define  SG_DIG_PARTNER_NEXT_PAGE	 0x00010000	/* If
							 * !MRADV_CRC16_SELECT */
...
#define MII_TG3_CTRL			0x09	/* 1000-baseT control
						 * register */
...
#define MII_TG3_DSP_RW_PORT		0x15	/* DSP coefficient read/write
						 * port */
...
#define MII_TG3_AUX_CTRL		0x18	/* auxilliary control
						 * register */
...
#define MII_TG3_EPHYTST_MISCCTRL	0x10	/* 5906 EPHY misc ctrl shadow
						 * register */
...
struct tx_ring_info {
	iovec_t iovec;
	u32_t prev_vlan_tag;
	phys_bytes mapping;	/* MINIX: physical address of iovec->iov_addr */
	int in_use;		/* Minix: We don't have null SKBs, so a flag
				 * is used instead. */
};
...
#define AUTONEG_INVALID		0xff
	...
	/* When we go in and out of low power mode we need to swap with this
	 * state. */
	int phy_is_low_power;
	u16_t orig_speed;
	u8_t orig_duplex;
	u8_t orig_autoneg;
	u32_t orig_advertising;
};

struct tg3_bufmgr_config {
	u32_t mbuf_read_dma_low_water;
	u32_t mbuf_mac_rx_low_water;
	u32_t mbuf_high_water;

	u32_t mbuf_read_dma_low_water_jumbo;
	u32_t mbuf_mac_rx_low_water_jumbo;
	u32_t mbuf_high_water_jumbo;

	u32_t dma_low_water;
	u32_t dma_high_water;
};
...
struct tg3_stats {
	...
	u64_t rx_octets;
	u64_t rx_fragments;
	u64_t rx_ucast_packets;
	u64_t rx_mcast_packets;
	u64_t rx_bcast_packets;
	u64_t rx_fcs_errors;
	u64_t rx_align_errors;
	u64_t rx_xon_pause_rcvd;
	u64_t rx_xoff_pause_rcvd;
	u64_t rx_mac_ctrl_rcvd;
	u64_t rx_xoff_entered;
	u64_t rx_frame_too_long_errors;
	u64_t rx_jabbers;
	u64_t rx_undersize_packets;
	u64_t rx_in_length_errors;
	u64_t rx_out_length_errors;
	u64_t rx_64_or_less_octet_packets;
	u64_t rx_65_to_127_octet_packets;
	u64_t rx_128_to_255_octet_packets;
	u64_t rx_256_to_511_octet_packets;
	u64_t rx_512_to_1023_octet_packets;
	u64_t rx_1024_to_1522_octet_packets;
	u64_t rx_1523_to_2047_octet_packets;
	u64_t rx_2048_to_4095_octet_packets;
	u64_t rx_4096_to_8191_octet_packets;
	u64_t rx_8192_to_9022_octet_packets;
	...
	u64_t tx_xon_sent;
	u64_t tx_xoff_sent;
	u64_t tx_flow_control;
	u64_t tx_mac_errors;
	u64_t tx_single_collisions;
	u64_t tx_mult_collisions;
	u64_t tx_deferred;
	u64_t tx_excessive_collisions;
	u64_t tx_late_collisions;
	u64_t tx_collide_2times;
	u64_t tx_collide_3times;
	u64_t tx_collide_4times;
	u64_t tx_collide_5times;
	u64_t tx_collide_6times;
	u64_t tx_collide_7times;
	u64_t tx_collide_8times;
	u64_t tx_collide_9times;
	u64_t tx_collide_10times;
	u64_t tx_collide_11times;
	u64_t tx_collide_12times;
	u64_t tx_collide_13times;
	u64_t tx_collide_14times;
	u64_t tx_collide_15times;
	u64_t tx_ucast_packets;
	u64_t tx_mcast_packets;
	u64_t tx_bcast_packets;
	u64_t tx_carrier_sense_errors;
	u64_t tx_discards;
	u64_t tx_errors;

	/* Statistics maintained by Receive List Placement. */
	u64_t dma_writeq_full;
	u64_t dma_write_prioq_full;
	u64_t rxbds_empty;
	u64_t rx_discards;
	u64_t rx_errors;
	u64_t rx_threshold_hit;

	/* Statistics maintained by Send Data Initiator. */
	u64_t dma_readq_full;
	u64_t dma_read_prioq_full;
	u64_t tx_comp_queue_full;

	/* Statistics maintained by Host Coalescing. */
	u64_t ring_set_send_prod_index;
	u64_t ring_status_update;
	u64_t nic_irqs;
	u64_t nic_avoided_irqs;
	u64_t nic_tx_threshold_hit;
};
...
struct coalesce {
	 ...
	 * If 0, only rx_max_coalesced_frames is used. */
	u32_t rx_coalesce_usecs;

	/* How many packets to delay an RX interrupt after a packet arrives.
	 * If 0, only rx_coalesce_usecs is used.  It is illegal to set both
	 * usecs and max frames to zero as this would cause RX interrupts to
	 * never be generated. */
	u32_t rx_max_coalesced_frames;

	/* Same as above two parameters, except that these values apply while
	 * an IRQ is being serviced by the host.  Not all cards support this
	 * feature and the values are ignored in that case. */
	u32_t rx_coalesce_usecs_irq;
	u32_t rx_max_coalesced_frames_irq;

	/* How many usecs to delay a TX interrupt after a packet is sent.  If
	 * 0, only tx_max_coalesced_frames is used. */
	u32_t tx_coalesce_usecs;

	/* How many packets to delay a TX interrupt after a packet is sent.
	 * If 0, only tx_coalesce_usecs is used.  It is illegal to set both
	 * usecs and max frames to zero as this would cause TX interrupts to
	 * never be generated. */
	u32_t tx_max_coalesced_frames;

	/* Same as above two parameters, except that these values apply while
	 * an IRQ is being serviced by the host.  Not all cards support this
	 * feature and the values are ignored in that case. */
	u32_t tx_coalesce_usecs_irq;
	u32_t tx_max_coalesced_frames_irq;

	/* How many usecs to delay in-memory statistics block updates.  Some
	 * drivers do not have an in-memory statistic block, and in such
	 * cases this value is ignored. This value must not be zero. */
	...
	u32_t use_adaptive_rx_coalesce;
	u32_t use_adaptive_tx_coalesce;

	/* When the packet rate (measured in packets per second) is below
	 * pkt_rate_low, the {rx,tx}_*_low parameters are used. */
	u32_t pkt_rate_low;
	u32_t rx_coalesce_usecs_low;
	u32_t rx_max_coalesced_frames_low;
	u32_t tx_coalesce_usecs_low;
	u32_t tx_max_coalesced_frames_low;

	/* When the packet rate is below pkt_rate_high but above pkt_rate_low
	 * (both measured in packets per second) the normal {rx,tx}_*
	 * coalescing parameters are used. */

	/* When the packet rate is (measured in packets per second) is above
	 * pkt_rate_high, the {rx,tx}_*_high parameters are used. */
	u32_t pkt_rate_high;
	u32_t rx_coalesce_usecs_high;
	u32_t rx_max_coalesced_frames_high;
	u32_t tx_coalesce_usecs_high;
	u32_t tx_max_coalesced_frames_high;

	/* How often to do adaptive coalescing packet rate sampling, measured
	 * in seconds.  Must not be zero. */
	u32_t rate_sample_interval;
};
...
typedef struct tg3 {
	...
	int mtu;
	 ...
	 u32_t(*irq_handler) (struct tg3 *);


	u32_t bar;		/* Base Address Register */
	int irq;
	int flags;		/* DISABLED, ENABLED, GOTREADV, promisq,
				 * broadcast etc. */
	...
	/* PCI related */
	int pci_seen;		/* TRUE iff device available */
	u8_t pci_bus;
	u8_t pci_dev;
	u8_t pci_func;

	/* 'large' items */
	int hook_id;		/* IRQ hook id at kernel */
	eth_stat_t stat;
	ether_addr_t mac_address;
	char name[sizeof("tg3#n")];



	/* begin "general, frequently-used members" cacheline section */

	/* If the IRQ handler (which runs lockless) needs to be quiesced, the
	 * following bitmask state is used.  The SYNC flag is set by non-IRQ
	 * context code to initiate the quiescence.
	 * 
	 * When the IRQ handler notices that SYNC is set, it disables interrupts
	 * and returns.
	 * 
	 * When all outstanding IRQ handlers have returned after the SYNC flag
	 * has been set, the setter can be assured that interrupts will no
	 * longer get run.
	 * 
	 * In this way all SMP driver locks are never acquired in hw IRQ
	 * context, only sw IRQ context or lower. */
	unsigned int irq_sync;

	/* SMP locking strategy:
	 * 
	 * lock: Held during reset, PHY access, timer, and when updating
	 * tg3_flags and tg3_flags2.
	 * 
	 * netif_tx_lock: Held during tg3_start_xmit. tg3_tx holds netif_tx_lock
	 * when it needs to call netif_wake_queue.
	 * 
	 * Both of these locks are to be held with BH safety.
	 * 
	 * Because the IRQ handler, tg3_poll, and tg3_start_xmit are running
	 * lockless, it is necessary to completely quiesce the chip with
	 * tg3_netif_stop and tg3_full_lock before reconfiguring the device.
	 * 
	 * indirect_lock: Held when accessing registers indirectly with IRQ
	 * disabling. */
	int slock;

	 u32_t(*read32) (struct tg3 *, u32_t);
	void (*write32) (struct tg3 *, u32_t, u32_t);
	 u32_t(*read32_mbox) (struct tg3 *, u32_t);
	void (*write32_mbox) (struct tg3 *, u32_t,
			       u32_t);
	...
	vir_bytes status_mapping;
	u32_t last_tag;
	u32_t last_irq_tag;

	/* begin "tx thread" cacheline section */
	void (*write32_tx_mbox) (struct tg3 *, u32_t,
				  u32_t);
	u32_t tx_prod;
	u32_t tx_cons;
	u32_t tx_pending;
	...
	vir_bytes tx_desc_mapping;

	/* begin "rx thread" cacheline section */
	void (*write32_rx_mbox) (struct tg3 *, u32_t,
				  u32_t);
	u32_t rx_rcb_ptr;
	u32_t rx_std_ptr;
	u32_t rx_jumbo_ptr;
	u32_t rx_pending;
	u32_t rx_jumbo_pending;
	...
	vir_bytes rx_std_mapping;
	u32_t rx_std_max_post;

	struct tg3_rx_buffer_desc *rx_jumbo;
	struct ring_info *rx_jumbo_buffers;
	vir_bytes rx_jumbo_mapping;

	struct tg3_rx_buffer_desc *rx_rcb;
	vir_bytes rx_rcb_mapping;

	u32_t rx_pkt_buf_sz;

	/* begin "everything else" cacheline(s) section */
	struct tg3_stats estats;
	struct tg3_stats estats_prev;
...
#define TG3_FLG2_ANY_SERDES		(TG3_FLG2_PHY_SERDES | TG3_FLG2_MII_SERDES)
...
/* Minux uses alarms */
	...
	u16_t timer_counter;
	u16_t timer_multiplier;
	u32_t timer_offset;
	u16_t asf_counter;
	u16_t asf_multiplier;

	/* 1 second counter for transient serdes link events */
	u32_t serdes_counter;
...
#define SERDES_AN_TIMEOUT_5714S		1
	...
	u32_t grc_mode;
	u32_t grc_local_ctrl;
	u32_t dma_rwctrl;
	u32_t coalesce_mode;
	u32_t pwrmgmt_thresh;

	/* PCI block */
	u32_t pci_chip_rev_id;
	u16_t pci_cmd;
	u8_t pci_cacheline_sz;
	u8_t pci_lat_timer;
...
#define TG3_VER_SIZE 32
	...
	u32_t nic_sram_data_cfg;
	u32_t pci_clock_ctrl;
	struct pci_dev *pdev_peer;

	/* This macro assumes the passed PHY ID is already masked with
	 * PHY_ID_MASK. */
...
#define TG3_NVRAM_SIZE_2MB		0x00200000
	...
	u32_t nvram_pagesize;
	u32_t nvram_jedecnum;
...
#define SST_25VF0X0_PAGE_SIZE		4098
	...
	struct coalesce coal;

	/* firmware info */
	const char *fw_needed;
	const struct firmware *fw;
	u32_t fw_len;		/* includes BSS */
} tg3_t;
...
#define MR_LINK_OK		0x80000000
	...
	u32_t ability_match_cfg;
	int ability_match_count;

	char ability_match, idle_match, ack_match;

	u32_t txconfig, rxconfig;
